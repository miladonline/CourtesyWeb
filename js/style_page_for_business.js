...
Allset.data.analytics = {
    init: function() {}
};
$(function() {
    Allset.data.analytics.init();
});
! function(a) {
...
Allset.data.teams = {
    ...
    initOpenMobileMenu: function() {
        $(".hamburger").click(function() {
            $(".top-nav_list").addClass('menu_open');
    ...
    initCloseMobileMenu: function() {
        $(".cross").click(function() {
            $(".top-nav_list").removeClass('menu_open');
    ...
    initScrollSection: function() {
        ...
        $('a[href^="#"]').off().on('click', function(e) {
            e.preventDefault();
            scrollHeight = $('.getstarted_form').offset().top - 100;
    ...
    initPhoneMask: function() {
        $('.getstarted_input.phone').mask("+9? (999) 999-9999", {
            placeholder: " "
        });
    },
    initSubmitApplicationForm: function() {
        var self = this,
            data;
        $('.getstarted_form button.btn-red').off().on('click', function(e) {
            e.preventDefault();
            $('.getstarted_input').removeClass('error');
            data = {
                "name": $('.getstarted_input[name="name"]').val(),
                "email": $('.getstarted_input[name="email"]').val(),
                "phone": $('.getstarted_input[name="phone"]').val(),
                "company_title": $('.getstarted_input[name="company_title"]').val(),
                "city": $('.getstarted_input[name="city"]').val(),
                "company_size": $('.getstarted_input[name="company_size"]:checked').val()
            };
            $.ajax({
                ...
                async: true,
                success: function() {
                    $('.section_title.getstarted_title').hide();
                    $('.getstarted_form').hide();
                    $('.section_title.thanks_info').show();
                },
                error: function(response) {
                    var r = JSON.parse(response.responseText).message.split(' | ');
                    $.each(r, function(index, value) {
                        var fieldName = value.split(':')[0];
                        $('.getstarted_input[name="' + fieldName + '"]').addClass('error');
                    });
                }
            });
        });
